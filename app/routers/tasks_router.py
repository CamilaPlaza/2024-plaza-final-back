...
def create_task(task: Task, user_data=Depends(verify_token)):
    return create_new_task(task)

@router.put("/{task_id}/status/{status}")
def change_status(task_id: str, status: str, user_data=Depends(verify_token)):
    return update_task_state(task_id, status)

@router.post("/assign")
def assign_task_to_employee(shift_employee: ShiftEmployee, user_data=Depends(verify_token)):

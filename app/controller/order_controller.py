...
def add_order_items(order_id: str, new_order_items_data: List[dict], total: str):
    ...
    for item in new_items:
        ...
        if isinstance(prod_res, dict) and "product" in prod_res:
            prod = prod_res["product"]
            ...
            price_f = _parse_float(str(prod.get("price")), "product_price")
        else:
            if item.product_price is None or item.product_price == "":
                raise HTTPException(status_code=400, detail=f"product_price required for product ID {item.product_id}")
